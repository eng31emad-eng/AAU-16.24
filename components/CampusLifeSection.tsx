...
export const CampusLifeSection = ({ initialData }: CampusLifeSectionProps) => {
  const { t, language } = useLanguage();
  const router = useRouter();
  const [campusLife, setCampusLife] = useState<CampusLifeItem[]>(initialData || []);
  ...
  useEffect(() => {
    if (!initialData) {
      ...
      setCampusLife(initialData.slice(0, 9));
    }
  }, [initialData]);
